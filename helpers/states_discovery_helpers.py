...
def Systemic_toxicity(patient_samples):
    ...
    fever_samples = []
    skin_look_samples = []
    allergic_state_samples = []
    for sample in patient_samples:
        if sample["Component"] == "Chills":
            chills_samples.append(sample)
        elif sample["Component"] == "Temperature":
            fever_samples.append(sample)
        elif sample["Component"] == "Skin assessment":
            skin_look_samples.append(sample)
        elif sample["Component"] == "Allergies":
            allergic_state_samples.append(sample)
    chills_samples = sorted(chills_samples, key=lambda x: x['Valid start time'], reverse=True)
    fever_samples = sorted(fever_samples, key=lambda x: x['Valid start time'], reverse=True)
    skin_look_samples = sorted(skin_look_samples, key=lambda x: x['Valid start time'], reverse=True)
    allergic_state_samples = sorted(allergic_state_samples, key=lambda x: x['Valid start time'], reverse=True)

    for i in range(len(fever_samples)):
        try:
        ...
        sample = {"Patient_name": fever_samples[i]['First name'] + ' ' + fever_samples[i]['Last name'],
                  "Concept_Name": "Systemic_toxicity",
                  "Valid Start Time": fever_samples[i]['Valid start time'],
                  "Valid End Time": fever_samples[i]['Valid end time'],
...
def hemoglobin(patient_samples, gender):
    ...
    result_abstraction.sort(key=lambda x: (x.get('Valid start time before', ''), x.get('Valid start time', '')))
...
def hematological(patient_samples, gender):
    ...
    hemoglobin_abstractions.sort(key=lambda x: parse_datetime(x['Valid start time before']))
    ...
    # Sort result_abstraction by start time
    result_abstraction.sort(key=lambda x: parse_datetime(x['Valid start time before']))
...
#         sample['Valid end time after'] = parse_datetime(sample['Valid end time after'])
#
#     # Resolve conflicts and merge in a single pass
#     merged_samples = []
#     current_sample = samples[0]
#
#     for next_sample in samples[1:]:
#         if current_sample['Valid end time after'] > next_sample['Valid start time before']:
#             # Calculate lengths and conflict
#             first_sample_length = (current_sample['Valid end time after'] - current_sample['Valid start time before']).total_seconds()
#             second_sample_length = (next_sample['Valid end time after'] - next_sample['Valid start time before']).total_seconds()
#             conflict_length = (current_sample['Valid end time after'] - next_sample['Valid start time before']).total_seconds()
#
#             # Calculate proportion and adjust times
#             proportion = first_sample_length / (first_sample_length + second_sample_length)
#             extra_time_first_sample = timedelta(seconds=conflict_length * proportion)
#
#             new_boundary = next_sample['Valid start time before'] + extra_time_first_sample
#             current_sample['Valid end time after'] = new_boundary
#             next_sample['Valid start time before'] = new_boundary
#
#         if current_sample['Value_State'] == next_sample['Value_State']:
#             # Merge samples with the same state
#             current_sample['Valid end time after'] = next_sample['Valid end time after']
#         else:
#             # Add the current sample to the result and move to the next
#             merged_samples.append(current_sample)
#             current_sample = next_sample
#
#     # Add the last sample
